...
\begin{enumerate}[{\bf BE1.}]
		...
		\begin{enumerate}[{\bf VP1.}]
			...
			\noindent\fbox{
				\parbox{0.85\textwidth}{
					\begin{itemize}
						\item {$S_{1}$:} System asks user for a pickup location and a destination point.
						\item {$E_{1}$:} Passenger inputs a pickup location and a destination point.
						\item {$S_{2}$:} System asks if passenger would like to carpool with nearby users travelling in the same general direction.
						\item {$E_{2}$:} Passenger decides whether to carpool with nearby users. \begin{itemize}
							\item[] $E_{2.1}$: User inputs their intent to carpool with nearby users. \textbf{\color{red}\textlangle Branch BE1.VP1.S3.1 \textrangle}
							\item[]$E_{2.2}$: User inputs their intent to not carpool with nearby users. \textbf{\color{red}\textlangle BE1.VP1.1.S3.2 \textrangle}
						\end{itemize}
						\item {$S_{3}$:} System calculates the most suitable path to take to get from pickup location to destination location and display the total cost of travel. \begin{itemize}
							\item[] $S_{3.1}$: System calculates a one-way route from pickup to destination location with no stops.
							\item[] $S_{3.2}$: System calculates a suitable route from pickup to destination with the capability of other users partaking in a carpool ride.
						\end{itemize}
						\item {$E_{3}$:} User decides whether to take the ride or not. \begin{itemize}
							\item[] $E_{3.1}$: User decides to reject the ride. \textbf{\color{red}\textlangle Branch EXIT \textrangle}
							\item[] $E_{3.2}$: User decides to accept the ride. \textbf{\color{red}\textlangle Branch to BE1.VP1.2.S1 \textrangle}
						\end{itemize}
					\end{itemize}
				}
			}
			...
			\noindent\fbox{
				\parbox{0.85\textwidth}{
					\begin{itemize}
						\item {$S_{1}$:} System notifies driver to pick up passenger.
						\item {$E_{1}$:} Driver picks up the passenger and drops them off at their destination.
					\end{itemize}
				}
			}
		\end{enumerate}
			...
			\parbox{0.85\textwidth}{
				\begin{itemize}
					...
					\item {$E_{3}$:} User decides whether to take the ride or not. \begin{itemize}
						...
						\item[] $E_{3.2}$: User decides to accept the ride. \textbf{\color{red}\textlangle Branch to BE1.VP1.2.S1 \textrangle}
					\end{itemize}
					\item {$S_{4}$:} System notifies driver to pick up passenger.
					\item {$E_{4}$:} Driver picks up the passenger and drops them off at their destination.
				\end{itemize}
			}
		}
	...
	\begin{enumerate}[{\bf VP1.}]
		...
		\noindent\fbox{
			\parbox{0.85\textwidth}{
				\begin{itemize}
					\item {$S_{1}$:} System attempts to authenticate the passenger and confirm that passenger has taken a ride previously. \begin{itemize}
						\item[] $S_{1.1}$: System successfully authenticates the passenger and displays a list of applicable (previous carpool partners) passengers to leave a review for \textbf{\color{red}\textlangle Branch to BE3.VP2.1. \textrangle}
						\item[]$S_{1.2}$: System fails to authenticate the passenger. \textbf{\color{red}\textlangle Branch EXIT\textrangle} 
					\end{itemize}
					\item {$E_{1}$:} Passenger selects an individual to leave a rating for.
					\item {$S_{2}$:} System receives response and prompts passenger for a message/rating for the selected individual. System waits for user response.
					\item {$E_{2}$:} Passenger responds to the system \begin{itemize}
						\item[] $E_{2.1}$: Passenger cancels the review process
						\item[]$E_{2.2}$: Passenger inputs a valid response (I.e. gives a rating and a potential message response) to the system
					\end{itemize}
					\item {$S_{3}$:} System calculates the most suitable path to take to get from pickup location to destination location and display the total cost of travel. \begin{itemize}
						\item[] $S_{3.1}$: System receives an invalid response (e.g., passenger cancels review process) \textbf{\color{red}\textlangle Branch EXIT \textrangle}
						\item[] $S_{3.2}$: System receives a valid response and updates the individual's profile with the new rating
					\end{itemize}
					\item {$E_{3}$:} Passenger is returned to ratings screen.
				\end{itemize}
			}
		}
		\item Driver \\
		...
		\noindent\fbox{
			\parbox{0.85\textwidth}{
				\begin{itemize}
					...
					\item {$S_{2}$:} System stores personal information of user and allows carpool services under the account.
				\end{itemize}
			}
		}
		\item Driver \\
