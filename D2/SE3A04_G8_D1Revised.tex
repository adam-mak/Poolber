...
\begin{enumerate}[{\bf BE1.}]
	\item Request a ride (Precondition: Passenger must be logged on)
		\begin{enumerate}[{\bf VP1.}]
			\item Requester \\
			\noindent\fbox{
				\parbox{0.85\textwidth}{
					\begin{itemize}
						\item {$S_{1}$:} System asks requester for a pickup location and a destination point and allows them to input a search preference (nearest cab, highest rating first, find specific offeror).
						\item {$E_{1}$:} Requester inputs a pickup location and a destination point, as well as any optional information.
	...
	\item Arrive at Destination (Precondition: Passenger is logged onto their account and are currently in a ride)
	\begin{enumerate}[{\bf VP1.}]
		\item Passenger (Requester and Offerer) \\
		\noindent\fbox{
			\parbox{0.85\textwidth}{
				\begin{itemize}
					\item {$S_{1}$:} System displays the price of the ride for each passenger (price depends on distance travelled). System allocates points to the passenger depending on how much distance they travelled. The system prompts the passenger and asks if the passenger wants to redeem a free ride for a certain amount of their current amount of points. 

                    \item {$E_{1}$:} Passenger decides whether to get a free ride. \begin{itemize}
						\item[] $E_{1.1}$: Passenger trades their points for a free ride.
						\item[] $E_{1.2}$: Passenger does not trade their points for a free ride. The system prompts the passenger with rating options for the other drivers 
      \textbf{\color{red}\textlangle Branch to VP1.E2\textrangle} 
					\end{itemize}
					
					\item {$S_{2}$:} System updates the displayed price of the ride to \$0. The system prompts the passenger with rating options for the other drivers.
					\item {$E_{2}$:} Passenger rates other passengers or chooses to skip.
				\end{itemize}
			}
		}
	\end{enumerate}
	{\bf Global Scenario (GS2):}\\
	\noindent\fbox{
		\parbox{0.85\textwidth}{
			\begin{itemize}
					\item {$S_{1}$:} System displays the price of the ride for each passenger (price depends on distance travelled). System allocates points to the passenger depending on how much distance they travelled. The system prompts the passenger and asks if the passenger wants to redeem a free ride for a certain amount of their current amount of points. 
                    ...
                    \item {$E_{1}$:} Passenger decides whether to get a free ride. \begin{itemize}
						\item[] $E_{1.1}$: Passenger trades their points for a free ride.
						\item[] $E_{1.2}$: Passenger does not trade their points for a free ride. The system prompts the passenger with rating options for the other drivers 
      \textbf{\color{red}\textlangle Branch to GS2.E2\textrangle} 
					\end{itemize}
	...
	\item Create account (Precondition: User is not logged into an account)
	\begin{enumerate}[{\bf VP1.}]
		\item Passenger \\
		\noindent\fbox{
			\parbox{0.85\textwidth}{
				\begin{itemize}
					\item {$S_{1}$:} System prompts user for personal information (Full name, phone number, email).
					\item {$E_{1}$:} User responds to the system. 

                    \item{$S_{2}$:} System checks if the submitted username matches any pre-existing account usernames. 
                    \begin{itemize}
						\item[] $S_{2.1}$: Submitted username matches a pre-existing account username. \textbf{\color{red}\textlangle Branch to VP1.S1\textrangle}
						\item[] $S_{2.2}$: Submitted username does not match a pre-existing account username. User's personal information is added to the database.
					\end{itemize}
				\end{itemize}
			}
		}
	\end{enumerate}
	{\bf Global Scenario (GS3):}\\
	\noindent\fbox{
		\parbox{0.85\textwidth}{
			\begin{itemize}
					\item {$S_{1}$:} System prompts user for personal information (Full name, phone number, email).
                    ...
                    \item{$S_{2}$:} System checks if the submitted username matches any pre-existing account usernames. 
                    \begin{itemize}
						\item[] $S_{2.1}$: Submitted username matches a pre-existing account username. \textbf{\color{red}\textlangle Branch to GS3.S1\textrangle}
						\item[] $S_{2.2}$: Submitted username does not match a pre-existing account username. User's personal information is added to the database.
					\end{itemize}
				\end{itemize}
		}
	}
	\item Remove account (Precondition: Passenger is logged into their account)
	\begin{enumerate}[{\bf VP1.}]
		\item Passenger \\
		\noindent\fbox{
			\parbox{0.85\textwidth}{
				\begin{itemize}
					\item {$S_{1}$:} System prompts passenger to confirm they want to remove their account.
					\item {$E_{1}$:} User decides whether they want to remove their account.
					\begin{itemize}
						\item[] $E_{2.1}$: User confirms they want to remove their account. 
						\item[] $E_{2.2}$: User decides not to remove their account. \textbf{\color{red}\textlangle Branch EXIT\textrangle}
					\end{itemize}
					\item {$S_{2}$:}  System removes the passengers account information from the database. 
				\end{itemize}
			}
		}
	\end{enumerate}
	{\bf Global Scenario (GS4):}\\
	\noindent\fbox{
		\parbox{0.85\textwidth}{
			\begin{itemize}
					\item {$S_{1}$:} System prompts passenger to confirm they want to remove their account.
	...
	\item Offer a ride (Pre-condition: Passenger is logged into their account)
	\begin{enumerate}[{\bf VP1.}]
		\item Offeror \\
		\noindent\fbox{
			\parbox{0.85\textwidth}{
				\begin{itemize}
					\item {$S_{1}$:} System asks offerors for information regarding the offer (destination, taxi ID, max number of customers to share with). 
					\item {$E_{1}$:} Offeror inputs the carpool information. 
                    \item {$S_{2}:$} System waits for a carpool request from a requester. The system provides user with the options to cancel the carpool offer (stop the carpool event entirely) or to begin the ride (begin the ride with the current accepted requesters).
                    \begin{itemize}
						\item[] $S_{2.1}$: System receives request from the requester. System forwards the request to the offeror of the carpool event. In addition, they are presented with changes to their trip fare, distance, total estimated travel time, a distance – trip fare ratio, and the updated route that they will be taking
						\item[] $S_{2.2}$: System receives a “cancel the carpool” response from the user.  \textbf{\color{red}\textlangle Branch EXIT\textrangle}
                            \item[] $S_{2.3}$: System receives a “begin the ride” response from user.
                            \textbf{\color{red}\textlangle Branch EXIT\textrangle}
					\end{itemize}
					\item {$E_{2}$:}  Offeror chooses whether or not they want to accept the carpool request. 
                    \item {$S_{3}$:}
                    \begin{itemize} System checks for offerors response.
						\item[] $S_{3.1}$: System adds the requester to the carpool event if the offeror accepts the request. The requester is notified that their request has been accepted. System asks offeror if they would like to close the carpool event/ begin the ride.
						\item[] $S_{3.2}$: The requester is notified that their request has been denied. System asks offeror if they would like to close the carpool event/ begin the ride 
					\end{itemize}
                    \item{$E_{3}$:} Offeror chooses whether or not to end the carpool event, begin the ride, or keep looking for more requests. 
                    \begin{itemize} 
						\item[] $E_{3.1}$: Offeror chooses begin the ride. 
                        \textbf{\color{red}\textlangle Branch EXIT\textrangle}
						\item[] $S_{3.2}$: Offeror chooses cancel carpool event.
                        \textbf{\color{red}\textlangle Branch EXIT\textrangle}
                        \item[] {$S_{3.3}$:} Offeror chooses to look for more requests.
                        \textbf{\color{red}\textlangle Branch VP1.S2\textrangle}
                        \end{itemize}
				\end{itemize}
			}
		}
	\end{enumerate}
	{\bf Global Scenario (GS5):}\\
	\noindent\fbox{
		\parbox{0.85\textwidth}{
			\begin{itemize}
					\item {$S_{1}$:} System asks offerors for information regarding the offer (destination, taxi ID, max number of customers to share with). 
                    ...
                    \begin{itemize} 
						\item[] $E_{3.1}$: Offeror chooses begin the ride. 
                        \textbf{\color{red}\textlangle Branch EXIT\textrangleUsers/}
						\item[] $S_{3.2}$: Offeror chooses cancel carpool event.
                        \textbf{\color{red}\textlangle Branch EXIT\textrangle}
                        \item[] {$S_{3.3}$:} Offeror chooses to look for more requests.
                        \textbf{\color{red}\textlangle Branch GS5.S2\textrangle}
                        \end{itemize}
				\end{itemize}
		}
	}
	\item Login to an account (Precondition: Passenger is currently not logged into an account)
	\begin{enumerate}[{\bf VP1.}]
		\item Passenger \\
		\noindent\fbox{
			\parbox{0.85\textwidth}{
				\begin{itemize}
					...
					\item {$E_{1}$:} Passenger responds to the system. 
					\item {$S_{2}$:} System authenticates login information.
					\begin{itemize}
						\item[] $S_{2.1}$: Customers login info matches an account and they are granted access. 
						\item[] $E_{1.2}$: Customers login info matches an account and they are granted access
      \textbf{\color{red}\textlangle Branch VP1.S1\textrangle}
					\end{itemize}
				\end{itemize}
			}
		}
	\end{enumerate}
	{\bf Global Scenario (GS6):}\\
	\noindent\fbox{
		\parbox{0.85\textwidth}{
			\begin{itemize}
					\item {$S_{1}$:} System prompts passenger for login information (username and password). 
					...
					\begin{itemize}
						\item[] $S_{2.1}$: Customers login info matches an account and they are granted access. 
						\item[] $E_{1.2}$: Customers login info matches an account and they are granted access
      \textbf{\color{red}\textlangle Branch S1\textrangle}
					\end{itemize}
				\end{itemize}
		}
	}
	\item Update account information (Precondition: User is logged onto their account)
	\begin{enumerate}[{\bf VP1.}]
		\item Passenger \\
		\noindent\fbox{
			\parbox{0.85\textwidth}{
				\begin{itemize}
					\item {$S_{1}$:} System displays their previous personal information form and prompts the passenger to make changes(Full name, phone number, email).
